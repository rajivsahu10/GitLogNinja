...
addCurrentCommit(){
    ...
    do 
        ...
        diffStr="${diffStr} \"locAdded\" : \"$locAdded\","
        diffStr="${diffStr} \"locDeleted\" : \"$locDeleted\","
        diffStr="${diffStr} \"srcFile\" : \"$srcFile\","
        diffStr="${diffStr} \"srcFileType\" : \"$srcFileType\""
        
        printf "{ \"index\" : { \"_index\" : \"gitindex\", \"_type\" : \"commitlog\"  } }\n" >>  $commitsJsonFile
        printf "${commitStr} ${diffStr} } \n"  >>  $commitsJsonFile
...
commitsJsonFile="${baseDir}/commits-bulkRestOutput-flat-${repoName}.json"
